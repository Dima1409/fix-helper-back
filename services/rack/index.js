const {RackSchema} = require("../../models/rack");
const cloudinary = require("cloudinary").v2;

const getAllRacks = async () => {
    let result
    result = await RackSchema.find({}).sort({name: 1});
    return result;
};

const getRacksByApplication = async (application) => {
    try {
        let result;
        result = await RackSchema.find({
            application: {$regex: application, $options: "i"}
        }).sort({name: 1});
        return result
    } catch (error) {
        console.error("Error fetching racks by application", error);
        throw error
    }
};
...
const createNewRack = async (name, type, kit, more, application, oem) => {
    ...
    let newRack;
    newRack = await RackSchema.create({
        name,
        type,
        kit,
        more,
...
const editRack = async (id, name, type, kit, more, application, oem) => {
    ...
    let updateRack;
    updateRack = await RackSchema.findOneAndUpdate({_id: id}, updates, {
        new: true,
    });
    return updateRack;
};
...
const addRackImage = async (id, {mainImage, mainImageId}) => {
    let result;
    result = RackSchema.findByIdAndUpdate(
        id,
        {
            mainImage,
            mainImageId,
...
const deleteImage = async (imgId) => {
    let result;
    result = await cloudinary.api.delete_resources([imgId], {
        type: "upload",
        resource_type: "image",
    });
    return result;
